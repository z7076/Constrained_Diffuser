...
class MazeRenderer:
    ...
    def renders(self, observations, conditions=None, title=None):
        ...
        ellipse_center_x = 5.8/12
        ellipse_center_y = 5/9
        # ellipse_width = 0.2
        # ellipse_height = 0.1
        ellipse_radius_x = 1/12
        ellipse_radius_y = 1/9

        theta = np.linspace(0, 2 * np.pi, 100)
        x = ellipse_radius_x * np.cos(theta) + ellipse_center_x
        y = ellipse_radius_y * np.sin(theta) + ellipse_center_y
        ax.plot(x, y, c='black', zorder=15) # Use ax.plot, grey color, zorder 15
        ax.fill(x, y, c='black', zorder=15) # Fill the ellipse
